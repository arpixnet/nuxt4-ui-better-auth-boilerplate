...
export const useAuthSession = () => {
  ...
  const loadSession = async () => {
    try {
      ...
      if (sessionData && sessionData.data) {
        ...
        if (isExpired.value) {
          logger.warn('auth-client', 'Session expired, redirecting to login')
      ...
      logger.error('auth-client', 'Error loading session', err)
  ...
  watch(isExpired, (expired) => {
    if (expired && session.value.data) {
      logger.warn('auth-client', 'Session expired during usage, redirecting to login')
      session.value = { data: null, error: null }
      router.push('/auth/login')
    }
  })
